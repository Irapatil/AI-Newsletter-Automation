...
import { useGenerateNewsletter } from "@/hooks/use-newsletter-queries";
import { formatDuration } from "@/lib/utils";

export function GeneratePage() {
  const mutation = useGenerateNewsletter();
  ...
  const hasStarted = mutation.isPending || mutation.isSuccess || mutation.isError;
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <Card className="overflow-hidden">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>
            Runs the full LangGraph pipeline: Orchestrator → 8 parallel collector agents →
            Aggregator → semantic Deduplication → Ranking → Newsletter Generator (GPT
            summarization) → HTML Formatter.
          </CardDescription>
        </CardHeader>
              ...
              {mutation.isSuccess && (
                <div className="flex w-full flex-col items-center gap-3 rounded-lg border border-success/30 bg-success/5 p-4">
                  <p className="text-sm font-medium text-success">
                    Newsletter generated in {formatDuration(mutation.data.executionTimeMs)}
                  </p>
